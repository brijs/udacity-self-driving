...
def accuracy(y_hat, Y):
    ...
    - acc [tensor]: accuracy
    """
    # calculate argmax. The index of max value. This corresponds to predicted classnum
    #  Also, shape = (N,); since axis=1 is collapsed
    argmax = tf.cast(tf.argmax(y_hat, axis=1), Y.dtype)

    # calculate acc: total_correct_predictions / total_samples
